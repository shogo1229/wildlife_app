...
class _Local_CameraState extends State<Local_Camera> {
  ...
  // 各過去セッションの展開状態を管理するマップ
  Map<String, bool> _expandedSessions = {};

  @override
  void initState() {
    super.initState();
    _selectedUserId = context.read<UserProvider>().getUserId(); // ユーザーIDを取得
    _loadSessions();
  }
  ...
  Future<void> _loadSessions() async {
    ...
    setState(() {
      ...
      // 展開状態を初期化（過去セッションのみ）
      for (var session in _traceSessions) {
  ...
  Widget build(BuildContext context) {
    // アクティブなセッションと過去のセッションに分類
    List<TraceSession> activeSessions =
        _traceSessions.where((s) => s.endTime == null).toList();
    List<TraceSession> pastSessions =
        _traceSessions.where((s) => s.endTime != null).toList();
  ...
  void _startTracing() {
    ...
    setState(() {
      ...
      // アクティブセッションは常に展開状態をtrueに設定（詳細を常に表示）
    });
